...
void establishment_guess(Stand& stand,Patch& patch) {
	...
	while (pftlist.isobj) {
		...
		if (establish(patch, stand.get_climate(), pft) && pft.lifeform == TREE) {
			nwoodypfts_estab++;
			estmax_total += pft.est_max; // f_js /SR
		}
		pftlist.nextobj();
	}
		...
		if (stand.pft[pft.id].active) {
			...
			if (establish(patch, stand.get_climate(), pft)) {
				...
				if (pft.lifeform==GRASS) {
					...
					//dprintf("%i: %f\n", date.year, est); // SR 14.07.16 advice from Jörg

					est *= pft.est_indiv / estmax_total;
					//est *= 0.15 / estmax_total; // fag_syl que_rob = 0.24 // Pic_abi abi_alb = 0.05

					//est = min(est, 200.0); // funktioniert nicht für abi alb
					//dprintf("%i: %f\n", date.year, est);
...
void mortality_guess(Stand& stand, Patch& patch, const Climate& climate, double fireprob) {
	...
	if (iffire) {
		...
		if (randfrac(stand.seed)<fireprob) {
			...
			while (vegetation.isobj) {
				Individual& indiv=vegetation.getobj();
        Patchpft& patchpft = patch.pft[indiv.pft.id]; // f_js_20170607
          ...
          // f_js_20170207
          double old_densindiv = indiv.densindiv;

          // f_js_20170703
          patchpft.cmort += mort_fire * (indiv.cmass_leaf + indiv.cmass_root + indiv.cmass_sap + indiv.cmass_heart - indiv.cmass_debt);
          ...
          if (indiv.pft.lifeform == TREE && negligible(indiv.densindiv)) {
            ...
            patchpft.diamclass_mortstemno[diamclass] += old_densindiv * patcharea ;
            // f_js_20170706: add the remaining rest, too
            patchpft.cmort += indiv.cmass_leaf + indiv.cmass_root + indiv.cmass_sap + indiv.cmass_heart - indiv.cmass_debt;
          } else if (indiv.pft.lifeform == TREE ) {
            double diam = pow(indiv.height / indiv.pft.k_allom2, 1.0 / indiv.pft.k_allom3);
            int diamclass = (int)(floor(diam / diamclasswidth));
            if (diamclass >= ndiamclass)
              diamclass = ndiamclass - 1;
            patchpft.diamclass_mortstemno[diamclass] += (old_densindiv - indiv.densindiv) * patcharea ;
          }
          // f_js_20170207-
	...
	while (vegetation.isobj) {
		Individual& indiv=vegetation.getobj();
    Patchpft& patchpft = patch.pft[indiv.pft.id]; // f_js_20170207
		// For this individual ...
      ...
      if (indiv.pft.lifeform == TREE) {
        ...
        patchpft.cmort += indiv.cmass_leaf + indiv.cmass_root + indiv.cmass_sap + indiv.cmass_heart - indiv.cmass_debt; // f_js_20170706
      }
        ...
        // f_js_20170705
        patchpft.cmort += mort * (indiv.cmass_leaf + indiv.cmass_root + indiv.cmass_sap + indiv.cmass_heart - indiv.cmass_debt);
...
//********Aktuell*********** WIP
//THINNING 
//Standard thinning based on tree diameter in ins file aka conctinous cover forestry //SR 
// Based on code Fredrik and I did in Lund + my own code
/*void thinning(Stand& stand, Patch& patch) {
	
	bool killed;
	double frac_left;
	double strength = 0.3; //30 % of the trees between lower and upper diameter limits are cut
	double frac_thinned = 0.0;
	
	const double harv_frac = 0.65; //Fraction of wood harvested
	
	
	// report fluxes variables
	double charvest_flux = 0.0;
	double charvested_products_slow = 0.0;
				
	double min_diam = 0.25;
	double max_diam = 0.50;
	...
	while (vegetation.isobj) {
		...
		Individual& indiv = vegetation.getobj();
		double diameter = pow(indiv.height / indiv.pft.k_allom2, 1.0 / indiv.pft.k_allom3); // diameter
		
		if (indiv.pft.lifeform == TREE && diameter > min_diam) { // added diameter > min_diam
			
			// Target diameter type of continous cover forestry
			if (diameter > max_diam) {  // if diameter is above the max diameter = kill the object
				...
				charvest_flux = (indiv.cmass_sap + indiv.cmass_heart -
					indiv.cmass_debt)*harv_frac;
				patch.pft[indiv.pft.id].litter_wood += (indiv.cmass_sap + indiv.cmass_heart -
					indiv.cmass_debt)*(1 - harv_frac); //Thinning with harvest	
				...
				if (negligible(indiv.densindiv)) {
					...
					frac_thinned = strength;
					frac_left = 1 - strength;

					patch.pft[indiv.pft.id].litter_leaf += indiv.cmass_leaf*frac_thinned;
					patch.pft[indiv.pft.id].litter_root += indiv.cmass_root*frac_thinned;
					charvest_flux = (indiv.cmass_sap + indiv.cmass_heart -
						indiv.cmass_debt)*frac_thinned*harv_frac;
					patch.pft[indiv.pft.id].litter_wood += (indiv.cmass_sap + indiv.cmass_heart -
						indiv.cmass_debt)*frac_thinned*(1 - harv_frac); //Thinning with harvest	
			...
			// test print
			dprintf("height: %7.3f, k_allom2: %7.3f, k_allom3: %7.3f \n", indiv.height, indiv.pft.k_allom2, indiv.pft.k_allom3);
			dprintf("Diameter: %7.3f removed fraction %4.2f \n", diameter, frac_thinned);
			

			// Reduce individual density and biomass on patch area basis
			// to account for loss of killed individuals
			/* dont know if you need this
			indiv.reduce_biomass(frac_thinned, 0.0);
			const double remaining = 1.0 - frac_thinned;
			
			indiv.densindiv *= remaining;
			indiv.cmass_leaf *= remaining;
			indiv.cmass_root *= remaining;
			indiv.cmass_sap *= remaining;
			indiv.cmass_debt *= remaining;
			indiv.cmass_heart *= remaining;*/
...
/*void precommercialthinning(Stand& stand, Patch& patch) {
	
	double frac_left;


	if (date.get_calendar_year() == plant_year + 20) {
		...
		charvest_flux = (indiv.cmass_sap + indiv.cmass_heart -
			indiv.cmass_debt)*pft.thinning_frac*harv_frac; // with or without harv_frac?
		patch.pft[indiv.pft.id].litter_wood += (indiv.cmass_sap + indiv.cmass_heart -
			indiv.cmass_debt)*pft.thinning_frac*(1 - harv_frac); //Thinning
...
void thinning(Stand& stand, Patch& patch) {
	...
	//const double harv_frac = 0.30; //Fraction of wood harvested
	//double harv_strength = 0.60;
	bool killed;
	...
	while (pftlist.isobj) {
		...
		if (pft.lifeform == TREE && (date.get_calendar_year() - plant_year) % pft.thinning_interv == 0) {
			...
			while (vegetation.isobj) {
				...
				if (indiv.id != -1 && indiv.alive) {
					if (indiv.pft.id == pft.id) {
						...
						//dprintf("JS_DEBUG %s: %f\n", (char*)pft.name, ba_total[ipft]);
						double diam = pow(indiv.height / indiv.pft.k_allom2, 1.0 / indiv.pft.k_allom3);
			...
			if (date.get_calendar_year() == plant_year + 20) {
				...
				patch.pft[indiv.pft.id].litter_wood += (indiv.cmass_sap + indiv.cmass_heart -
					indiv.cmass_debt)*(1 - pft.thinning_frac); //Thinning
				...
				frac_left = 1 - pft.thinning_frac;
						...
						if (pft.thinning_regime == ABOVE && diam >= pft.min_diam) {
							...
							if (diam > pft.max_diam) {  // if diameter is above the max diameter = kill the object
								...
								//patch.pft[indiv.pft.id].litter_wood += (indiv.cmass_sap + indiv.cmass_heart -
								//	indiv.cmass_debt)*(1 - pft.harvest_frac); //Thinning with harvest	
...
// Thinning based on Height (EXPERIMENTAL!)
// Based on Jörgs Code
/*void thinning(Stand& stand, Patch& patch) {
	if (plant_year == 0)
		return;
	...
	double min_height = 5; //fag 17
	double max_height = 15; //fag 25
			...
			while (vegetation.isobj) {
				...
				if (indiv.id != -1 && indiv.alive) {
					if (indiv.pft.id == pft.id) {
						...
						if (pft.thinning_regime == ABOVE && indiv.height >= min_height) {

							// Target diameter type of continous cover forestry
							if (indiv.height > max_height) {  // if height is above the max height = kill the object
							...
							//dprintf("Height: %7.3f removed fraction %4.2f \n", indiv.height, pft.thinning_frac);

							patch.fluxes.report_flux(Fluxes::HARVESTC, charvest_flux);

						}
						else if (pft.thinning_regime == BELOW && indiv.height <= max_height) {

							if (indiv.height < min_height) {
								...
								charvest_flux += (indiv.cmass_sap + indiv.cmass_heart -
									indiv.cmass_debt)*pft.thinning_frac;
								patch.pft[indiv.pft.id].litter_wood += (indiv.cmass_sap + indiv.cmass_heart -
									indiv.cmass_debt)*pft.thinning_frac; //Thinning with harvest
						...
						if (!indiv.alive) vegetation.nextobj();
...
void vegetation_dynamics(Stand& stand,Patch& patch) {
	...
	if (vegmode == POPULATION) {
		...
		//SR_20161123
		if (date.get_calendar_year() == plant_year - 1) {
			disturbance(patch, 1.0);
      return;
    }
		...
		thinning(stand, patch);
		if (date.get_calendar_year() >= plant_year)
		finalharvest(stand, patch);
		//SR_20161205
		//finalharvest(stand, patch);

		// Management check original
		/*
		int first_cut = 450;
		int cut_int = 50; //SR 20161124 thinning_interval from ins file
		div_t cut_check;
				
		int patch_order = (int)(patch.id*cut_int*1.0 / (1.0*npatch));
		cut_check = div(date.year - first_cut - patch_order, cut_int);
		if (date.year >= first_cut && cut_check.rem == 0) {
			dprintf("thinning, %i \n",date.year);
			thinning(stand,patch);			
			patch.age++;
			return; //No disturbance, mortality or establishment the thinning year 
		...
		/*if (ifdisturb && patch.age && !during_century_solvesom && date.year<first_cut) {*/ // SR_20161125 needed for Stefan thinning
