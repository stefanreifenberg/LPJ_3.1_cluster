  ...
  void ProfoundOutput::outannual(Gridcell& gridcell) {
    ...
    if (date.get_calendar_year() >= plant_year) {
      ...
      for (int i = 0; i < ndiamclass; i++) {
        ...
        //out.add_value(out_diamclass_mai,        (i + 1) * diamclasswidth);
        out_diam.add_value(out_diamclass_ba,         100.0 * (i + 1) * diamclasswidth); // cm
        //out.add_value(out_diamclass_harv,       (i + 1) * diamclasswidth);
        ...
        //out.add_value(out_diamclass_vol,        (i + 1) * diamclasswidth);
        ...
        //out.add_value(out_dbh_domhei, -999.9);
        //out.add_value(out_dom_height, -999.9);
          if (diamclass_stemno[pftname][i] > 0) {
            //dprintf("JS_DEBUG: %i %s %i %f %f\n", date.get_calendar_year(), pftname, i, diamclass_harvstemno[pftname][i], diamclass_stemno[pftname][i]);
