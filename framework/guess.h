...
///
/// \author Ben Smith
/// $Date: 2015-04-28 16:48:02 +0200 (Di, 28 Apr 2015) $
///
///////////////////////////////////////////////////////////////////////////////////////
...
/// f_js_20160517 thinnig regime SR_20161205
typedef enum { NONE, BELOW, ABOVE, COMBINED } thinningregime;
...
public:
	...
	/// added for Fredriks cfinput.cpp SR 280316
	double temp_min;

	/// added for Fredriks cfinput.cpp SR 280316
	double ppfd_mean_month;
...
public:
	...
	int thinning_interv; // years SR_20161124
	double min_diam; //
	double max_diam; //
	int rotation_interv; // harvest year
	double thinning_frac; // as percent of BA
	double harvest_frac; // as percent of BA
	double est_indiv; // number of individuals planted
	thinningregime thinning_regime; // so far below (start from the smallest) and above (start from the largest)
...
public:
	...
	void updateSolveSOMvalues(const int& nyr
) {
		
		solvesom_end = static_cast<int>(0.8 * nyear_spinup);
		solvesom_begin = static_cast<int>(0.7 * nyear_spinup);
...
public:
	...
	double litter_repr;
	/// litter for wood SR 02.06.16
	double litter_wood;

  // f_js_20170206
  std::vector<double> diamclass_harv;
  std::vector<double> diamclass_harvstemno;
  std::vector<double> diamclass_mortstemno;

  // f_js_20170704: for additional column in cflux.out (mortality flux not associated with harvest)
  double cmort;
	...
	Patchpft(int i,Pft& p):id(i),pft(p) {
		...
		litter_repr  = 0.0;
		litter_wood = 0.0; // SR 02.06.

    // f_js_20170206
    diamclass_harv.assign(ndiamclass, 0.0);
    diamclass_harvstemno.assign(ndiamclass, 0.0);
    diamclass_mortstemno.assign(ndiamclass, 0.0);
